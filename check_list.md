...
- 랜딩페이지 주소: mindfitness-online.com/[페이지명]
...
9. 폼 제출 시, 메타의 **이벤트 매칭 품질(Event Match Quality)** 향상을 위해 성함(`name`)과 연락처(`phone`) 필드를 반드시 백엔드 CAPI 전송 시 함께 실어서 넘길 것! (서버 CAPI에서 이름은 `fn`, 연락처는 `ph` 매개변수로 SHA-256 해싱 매핑 처리됨)
...
#### 규칙 C. 이벤트 매칭 품질(Event Match Quality) 향상을 위한 고객 정보 매핑
* 메타 광고 대시보드의 매칭 품질 점수를 끌어올리기 위해, 고객이 폼에 입력한 개인정보를 서버 CAPI 전송 시 포함해야 합니다.
* **이름 입력칸:** 메타의 **`fn` (First Name)** 필드와 연결합니다. (서버 전송 전 공백 제거 및 소문자 변환 후 SHA-256 해싱 처리)
* **전화번호 입력칸:** 메타의 **`ph` (Phone)** 필드와 연결합니다. (서버 전송 전 숫자 이외의 문자 제거 후 SHA-256 해싱 처리)
* 프론트엔드 폼에서 `/api/capi`를 호출할 때 `name`과 `phone` 필드를 함께 전송하며, 백엔드(`lib/meta-capi.ts`의 `sendCustomEventCAPI`)에서 이를 해싱하여 `user_data` 페이로드에 실어 보냅니다.
...
- 예시 (한림점 케어멤버십): `https://mindfitness-online.com/api/revalidate?path=/hl-caremembership&secret=mindfitness123!`
